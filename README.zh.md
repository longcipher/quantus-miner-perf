...
> 与官方矿工协议完全一致，只是更快。
> **CPU 最高 2.6× · GPU 最高 1.59× · CPU/GPU 分开上报 · 更低的宿主开销。**
...
- **NVIDIA 上的原生 CUDA 后端**：u32 limb 的 Goldilocks 内核、按架构编译的 cubin、
  自动调优的 launch 参数——RTX 3090 Ti 上 164.54M → **262.05M H/s（1.59x）**。
...
| 用例 | 官方 | perf | 提速 |
|------|----------|------|---------|
| CPU 1 线程 | 179.26K H/s | 432.80K H/s | **2.41x** |
| CPU 32 线程 | 3.14M H/s | 7.90M H/s | **2.52x** |
| GPU 1 张卡 | 164.54M H/s | 262.05M H/s | **1.59x** |
| 合计（32 CPU + 1 GPU） | 169.05M H/s | 267.39M H/s | **1.58x** |

**CPU 约快 2.4–2.5 倍，GPU 约快 1.6 倍**，因此合计提升 **1.58x**，而不再是此前
