...
> **大陆矿工请走中转。** 直连矿池 IP `46.4.66.214` 在大陆被墙，因此我们提供了
> 中转/代理 **`57.180.173.214`**。在大陆挖矿请指向中转；海外用户可直连
> `46.4.66.214`（通常更快）。两种方式的 TLS 证书指纹完全一致，其余无需改动。

| | 端点 | PPLNS | Solo |
|---|---|---|---|
| `longpool` | 内置（默认走中转 `57.180.173.214`） | `--mode pplns` | `--mode solo` |
| `serve` | 手动填写 | `46.4.66.214:9900`（中转 `57.180.173.214:9900`） | `46.4.66.214:9901`（中转 `57.180.173.214:9901`） |

### 方式一：longpool 一键模式（推荐，开箱即用）

`longpool` **默认走中转 `57.180.173.214`**（IP 已内置在二进制内），大陆矿工直接
跑下面命令即可，无需填写任何连接参数。海外用户若想直连，传
`--pool-addr 46.4.66.214:9900`（PPLNS）或 `46.4.66.214:9901`（Solo）。两种方式的
TLS 指纹相同，同样无需手动填写。

**大陆 — PPLNS 模式**（经中转 `57.180.173.214`，默认，收益按份额分配）：
...
**大陆 — Solo 模式**（经中转 `57.180.173.214`，独立出块，出块者扣除手续费后获得整块奖励）：
...
- `--pool-addr`：矿池端点 `host:port`。默认走中转 `57.180.173.214`（对大陆友好）；海外用户传 `46.4.66.214:9900`（PPLNS）/ `46.4.66.214:9901`（Solo）直连
...
`--node-addr` 默认是直连端点 `46.4.66.214`（海外/更快）。**大陆矿工请把它换成中转
`57.180.173.214`** —— 端口仍是 9900/9901，指纹不变。
...
**Solo 模式：** 把 `--node-addr` 端口改成 `9901`，其余参数保持不变（直连
`46.4.66.214:9901` 或中转 `57.180.173.214:9901`）。
